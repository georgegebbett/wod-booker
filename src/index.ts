                    ...
                    if (wasConfirmed && cls.is_bookable && !cls.attending && !cls.waitlisted) {
                        ...
                        if (confirmation) {
                            ...
                            if (confirmation.thread_id) {
                                await sendMessage(
                                    confirmation.thread_id,
                                    `✅ Booked ${cls.title} for ${date.toLocaleString('en-GB', {hour: '2-digit', minute: '2-digit'})}`,
                                    config.discord_bot_token,
                                );
                            } else {
                                await sendMessage(
                                    config.discord_channel_id,
                                    `✅ Booked ${cls.title} for ${user.name} on ${date.toLocaleString('en-GB', {hour: '2-digit', minute: '2-digit'})}`,
                                    config.discord_bot_token,
                                );
                            }
                        }
